...
try:
    from ..quant import build_model_load_kwargs
...
class DecisionModel(nn.Module):
    def __init__(self, name, tok, device, lora=None, revision=None, attn=None, head_dim=256, option_isolation=False, special_embeddings=False, lora_targets="all", dtype=torch.float32, quantization=None):
        super().__init__()
        if attn is not None and not isinstance(attn, str):
            raise TypeError(f"attn must be a string or None, not {type(attn).__name__}")
        ...
        load_kwargs = build_model_load_kwargs(quantization, device, dtype)
