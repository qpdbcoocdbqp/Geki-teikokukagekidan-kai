...
The image includes Python 3.12, `uv`, and Google Chrome. From the directory containing the `jev-ultrafast` repository, build it with:

```bash
docker build --platform linux/amd64 -t jev-browse -f jev-ultrafast/Dockerfile .
docker run --rm -p 8766:8766 --env-file jev-ultrafast/.env jev-browse
```

Then open **http://127.0.0.1:8766**. Google Chrome's official Linux package is amd64-only, so ARM hosts should keep the `--platform linux/amd64` option (Docker Desktop can emulate it).

Test the running container without making model API calls:

```bash
python jev-ultrafast/scripts/test_demo_client.py
```

To replace the TypeSafe decision policy with a local OpenAI-compatible model, set these values in the `.env` passed to Docker:

```dotenv
JEV_MODEL_PROVIDER=openai
JEV_MODEL_BASE_URL=http://host.docker.internal:19001/v1
JEV_MODEL_API_KEY=local
...
```bash
python jev-ultrafast/scripts/test_demo_client.py --run \
  --scenario research \
