...
5. **Full-Sequence Scoring**: Every token in every multi-token candidate contributes to its score. Mean token log-probability avoids automatically penalizing labels that tokenize into more pieces, and a final softmax produces one normalized distribution across choices.
...
The PyTorch engine defaults to `Qwen/Qwen2.5-1.5B-Instruct`. Override `MODEL_ID` to use another text decoder model supported by Transformers `AutoModelForCausalLM`.

All inference paths render messages through the selected tokenizer's `apply_chat_template()` and continue an assistant JSON prefill. Models that reject a dedicated `system` role automatically receive the same instruction at the start of the user message. A compatible model must provide a chat template with assistant-prefill support, full vocabulary logits, and a reusable KV cache.

```bash
MODEL_ID=Qwen/Qwen2.5-3B-Instruct python3 -m uvicorn server.app:app --host 0.0.0.0 --port 8000
```
